...
impl ConfigBootstrap {
    ...
    pub fn initialize_logging(&self) -> Result<(), ConfigError> {
        ...
        if &current_dir != &self.base_path {
            if !std::env::set_current_dir(&self.base_path).is_ok() {
                println!(
        ...
        let result = match initialize_logging(&self.log_config) {
            true => Ok(()),
        ...
        if &current_dir != &std::env::current_dir().unwrap_or_default() {
            if !std::env::set_current_dir(&current_dir).is_ok() {
                println!(
                    "Working directory could not be changed back to {} after logging has been initialized. New \
